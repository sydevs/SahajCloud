...
| `EventVerificationEmail.tsx` | Manager/region event-verification reminder — coloured alert callout keyed on `ReminderLevel`. Also carries the listing-quality recommendations (#611): a `Suggested improvements` section built from `EventSuggestion[]`, worded by the `@/lib/eventQuality` registry (never inlined here) and shown to the **event manager only**. Absent or empty renders the email byte-identically to one with no open items. |
