...
| `PostEventFollowUpEmail.tsx` | Registrant follow-up after a session they attended (#626) — Sahaj Atlas brand. Built from composable `sections` rather than a fixed body, so later follow-up kinds (feedback forms, event promotion) can be added without another template. Today's only section is the **feedback ask** — "did this class take place?" with confirm/deny buttons — which is emitted only for an event that is still published and `unverified`; with no sections the job sends nothing and leaves its ledger unstamped, so those registrations stay eligible for a future follow-up type. Sent by the `SendPostEventFollowUps` job. |
