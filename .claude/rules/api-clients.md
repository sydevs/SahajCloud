...
assertion itself, right after `requireActiveClient` — `GET /api/atlas/seo` does,
and its own forwarded reads *would* fire the hook, but only after the handler
has already begun work. Don't lean on an incidental collection read to trigger the
