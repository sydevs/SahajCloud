...
| --- | --- | --- | --- |
| Schema/contract step | Payload migrations: attempt-then-fallback (`timeout 30 pnpm payload migrate:create <name> --skip-empty < /dev/null`; hand off on exit 124) per `.claude/rules/migrations.md` | `pnpm types:cms` + `pnpm types:openapi` contract refresh | `pnpm types:cms` (consumes SahajCloud types; no migrations) |
...
| Security-review trigger paths | access plugins, Clients/Managers, endpoints, storage, webhooks, payload.config | api config, widget + app entry, host-fragment routing, href/URL guards, lexical/HTML sinks, injected CSS + its scoping, privacy + reporting seams, deps + patches, env, vite config | `server/`, `pages/preview/`, wrangler/vite/sentry configs, `scripts/`, env files |
