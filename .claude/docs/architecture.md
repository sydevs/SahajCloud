...
- **Lectures** (`src/collections/resources/Lectures.ts`) - Lecture video content integrated with Nirmala Vidya API. Creation requires a Vimeo URL (`nirmalVidyaVimeoUrl`) which triggers server-side API fetch to auto-populate title, videoUrl (HLS), and thumbnail. Includes `lastRefreshed` timestamp and a custom `GET /:id/refresh` endpoint with `RefreshLectureButton` admin component for re-fetching metadata. `subtitlesUrl` is manually editable (API subtitle support pending).
