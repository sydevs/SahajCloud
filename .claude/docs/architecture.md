...
| `audiencesForUser` | `/api/audiences/for-user` | resolves the eligible audience IDs for a user from their progress data (`pathProgress`, `meditationsPerWeek`, `totalMeditationsViewed`, `totalLecturesViewed` — required integers) and required context (`country` ISO alpha-2, `timezone` IANA name). All six params are required. Progress audiences matched via SQL WHERE; context audiences matched via JS filter (country gate, schedule gate, daytime gate). Returns combined deduplicated sorted IDs. Mobile clients call once per state change and pass the result as `audiences` to the data endpoints below. `Cache-Control: public, max-age=300, s-maxage=300`. |
...
- **AppCards** — mobile cards with `image` relationship, type (app-page / content / external), countdown checkbox, `audiences` hasMany (OR semantics — shown if any overlap), `conditions` hasMany (AND semantics — all context audience IDs must be in the caller's resolved list), weight (1–5).
...
- **Audiences** — reusable visibility/targeting rules referenced by AppCards and Lectures. `type` discriminator: `progress` (SQL WHERE on native range fields — `pathProgress`, `meditationsPerWeek`, `totalMeditationsViewed`, `totalLecturesViewed`) or `context` (JS filter on `country` hasMany select, `eventTime` time-of-day gate, `schedule` recurrence gate). Three bidirectional joins: `lectures`, `appCards` (progress audiences), `appCardConditions` (context audiences). Rule eval lives on `/api/audiences/for-user`; data endpoints take pre-resolved IDs. `isEventInUserDaytime` and `isScheduleActiveNow` helpers in `src/lib/audiences/`.
