...
- **Lectures** (`src/collections/resources/Lectures.ts`) - Lecture video content integrated with Nirmala Vidya API. Creation requires a Vimeo URL (`nirmalVidyaVimeoUrl`) which triggers server-side API fetch to auto-populate title, videoUrl (HLS), thumbnail, and per-locale `subtitlesUrl`. Includes `startTime` and `endTime` number fields (stored as seconds) for defining video excerpt ranges, displayed as HH:MM:SS in the admin UI via a custom `TimestampInput` component. Cross-field validation ensures endTime > startTime. Has `tags` hasMany relationship to `lecture-tags` for categorization and targeting.
...
- **LectureTags** (`src/collections/tags/LectureTags.ts`) - Non-upload collection for lecture categorization with internal `label` field (non-localized), JSON-based **targeting rules** (`rulesField`) for user progress-based targeting (`pathProgress`, `totalMeditationsViewed`, `totalLecturesViewed` — all range type), and bidirectional join to lectures via `tags` relationship. Visible only in `wemeditate-app` project.
