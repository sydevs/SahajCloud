...
- **Meditations** (`src/collections/content/Meditations.ts`) - Guided meditation content with audio files, **type select** (quick/daily/lesson with `quick` as default), **timings multi-select** (morning/afternoon/evening/night) for time-based scheduling on quick/daily types, **duration** (stored in seconds, auto-extracted from audio via `music-metadata` in `beforeChange` hook), **durationMinutes** (virtual, computed from duration), frame relationships with timestamps, locale-specific content filtering via `beforeOperation` hook (`filterMeditationsByLocale`), 4 reverse join fields (`asMorningMeditation`, `asAfternoonMeditation`, `asEveningMeditation`, `asNightMeditation`) showing which MeditationTags reference this meditation, drafts system with scheduled publishing, and beforeChange validation requiring frames for publishing
