...
- **Audiences** — reusable visibility/targeting rules referenced by AppCards and Lectures. `type` discriminator: `progress` (SQL WHERE on native range fields — `pathProgress`, `meditationsPerWeek`, `totalMeditationsViewed`, `totalLecturesViewed`) or `context` (JS filter on `country` hasMany select — country gate only). Three bidirectional joins: `lectures`, `appCards` (progress audiences), `appCardConditions` (context audiences). Rule eval lives on `/api/audiences/for-user`; data endpoints take pre-resolved IDs.
