...
| A Payload endpoint for a resource **no** collection owns (`/api/contact-admin`, `/api/atlas/seo`, `/api/atlas/sitemap`); registered on `config.endpoints`. Rare — it forgoes the usage plugin's beforeOperation hooks, so origin enforcement must be called by hand | `src/endpoints/*.ts` — see `.claude/rules/endpoints.md`                    |
