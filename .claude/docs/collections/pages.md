...
- `lectureFilters` (relationship to viewer-rules, hasMany, minRows: 1) - Visible when type is lectures. Query string uses `where[audience][in]`.
- `lectureClipFilters` (relationship to viewer-rules, hasMany, minRows: 1) - Visible when type is lecture-clips. Query string uses `where[audience][in]`.
