...
// per-suite bootstrap (buildConfig + getPayload + Drizzle push) ~3–4× slower —
// enough to blow the normal 60s/120s timeouts. `pnpm test:coverage` is an
// opt-in, heavyweight run (≈20 min), so grant it much larger timeouts. Detected
// from argv since Vitest doesn't expose the coverage flag in config.
    ...
    projects: [
      ...
      },
      {
        ...
        test: {
          ...
          testTimeout: isCoverage ? 300000 : 60000,
