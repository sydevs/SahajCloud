...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`app_cards_timings\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`app_cards_timings_order_idx\` ON \`app_cards_timings\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`app_cards_timings_parent_idx\` ON \`app_cards_timings\` (\`parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_app_cards_v_version_timings\` (
  	\`order\` integer NOT NULL,
  	\`parent_id\` integer NOT NULL,
  	\`value\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_app_cards_v_version_timings_order_idx\` ON \`_app_cards_v_version_timings\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`_app_cards_v_version_timings_parent_idx\` ON \`_app_cards_v_version_timings\` (\`parent_id\`);`)
  ...
  await db.run(sql`ALTER TABLE \`_app_cards_v\` ADD \`version_label\` text;`)
  await db.run(sql`ALTER TABLE \`_app_cards_v\` ADD \`version_default_icon_id\` integer REFERENCES images(id);`)
  await db.run(sql`ALTER TABLE \`_app_cards_v\` ADD \`version_default_alignment\` text;`)
  await db.run(sql`ALTER TABLE \`_app_cards_v\` ADD \`version_starting_soon_icon_id\` integer REFERENCES images(id);`)
  await db.run(sql`ALTER TABLE \`_app_cards_v\` ADD \`version_starting_soon_alignment\` text;`)
  await db.run(sql`ALTER TABLE \`_app_cards_v\` ADD \`version_live_now_icon_id\` integer REFERENCES images(id);`)
  await db.run(sql`ALTER TABLE \`_app_cards_v\` ADD \`version_live_now_alignment\` text;`)
  ...
  await db.run(sql`ALTER TABLE \`app_cards\` ADD \`label\` text;`)
  await db.run(sql`ALTER TABLE \`app_cards\` ADD \`default_icon_id\` integer REFERENCES images(id);`)
  await db.run(sql`ALTER TABLE \`app_cards\` ADD \`default_alignment\` text;`)
  await db.run(sql`ALTER TABLE \`app_cards\` ADD \`starting_soon_icon_id\` integer REFERENCES images(id);`)
  await db.run(sql`ALTER TABLE \`app_cards\` ADD \`starting_soon_alignment\` text;`)
  await db.run(sql`ALTER TABLE \`app_cards\` ADD \`live_now_icon_id\` integer REFERENCES images(id);`)
  await db.run(sql`ALTER TABLE \`app_cards\` ADD \`live_now_alignment\` text;`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`app_cards_timings\`;`)
  await db.run(sql`DROP TABLE \`_app_cards_v_version_timings\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_app_cards\`("id", "type", "default_destination", "default_app_page", "default_lecture_id", "default_album_id", "default_meditation_id", "default_image_id", "default_overlay", "starting_soon_enabled", "starting_soon_threshold", "starting_soon_destination", "starting_soon_app_page", "starting_soon_lecture_id", "starting_soon_album_id", "starting_soon_meditation_id", "starting_soon_image_id", "starting_soon_overlay", "live_now_enabled", "live_now_threshold", "live_now_destination", "live_now_app_page", "live_now_lecture_id", "live_now_album_id", "live_now_meditation_id", "live_now_image_id", "live_now_overlay", "schedule_first_date", "schedule_firstdate_tz", "schedule_end_time", "schedule_recurrence_type", "schedule_interval", "weight", "updated_at", "created_at", "_status") SELECT "id", "type", "default_destination", "default_app_page", "default_lecture_id", "default_album_id", "default_meditation_id", "default_image_id", "default_overlay", "starting_soon_enabled", "starting_soon_threshold", "starting_soon_destination", "starting_soon_app_page", "starting_soon_lecture_id", "starting_soon_album_id", "starting_soon_meditation_id", "starting_soon_image_id", "starting_soon_overlay", "live_now_enabled", "live_now_threshold", "live_now_destination", "live_now_app_page", "live_now_lecture_id", "live_now_album_id", "live_now_meditation_id", "live_now_image_id", "live_now_overlay", "schedule_first_date", "schedule_firstdate_tz", "schedule_end_time", "schedule_recurrence_type", "schedule_interval", "weight", "updated_at", "created_at", "_status" FROM \`app_cards\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new__app_cards_v\`("id", "parent_id", "version_type", "version_default_destination", "version_default_app_page", "version_default_lecture_id", "version_default_album_id", "version_default_meditation_id", "version_default_image_id", "version_default_overlay", "version_starting_soon_enabled", "version_starting_soon_threshold", "version_starting_soon_destination", "version_starting_soon_app_page", "version_starting_soon_lecture_id", "version_starting_soon_album_id", "version_starting_soon_meditation_id", "version_starting_soon_image_id", "version_starting_soon_overlay", "version_live_now_enabled", "version_live_now_threshold", "version_live_now_destination", "version_live_now_app_page", "version_live_now_lecture_id", "version_live_now_album_id", "version_live_now_meditation_id", "version_live_now_image_id", "version_live_now_overlay", "version_schedule_first_date", "version_schedule_firstdate_tz", "version_schedule_end_time", "version_schedule_recurrence_type", "version_schedule_interval", "version_weight", "version_updated_at", "version_created_at", "version__status", "created_at", "updated_at", "snapshot", "published_locale", "latest") SELECT "id", "parent_id", "version_type", "version_default_destination", "version_default_app_page", "version_default_lecture_id", "version_default_album_id", "version_default_meditation_id", "version_default_image_id", "version_default_overlay", "version_starting_soon_enabled", "version_starting_soon_threshold", "version_starting_soon_destination", "version_starting_soon_app_page", "version_starting_soon_lecture_id", "version_starting_soon_album_id", "version_starting_soon_meditation_id", "version_starting_soon_image_id", "version_starting_soon_overlay", "version_live_now_enabled", "version_live_now_threshold", "version_live_now_destination", "version_live_now_app_page", "version_live_now_lecture_id", "version_live_now_album_id", "version_live_now_meditation_id", "version_live_now_image_id", "version_live_now_overlay", "version_schedule_first_date", "version_schedule_firstdate_tz", "version_schedule_end_time", "version_schedule_recurrence_type", "version_schedule_interval", "version_weight", "version_updated_at", "version_created_at", "version__status", "created_at", "updated_at", "snapshot", "published_locale", "latest" FROM \`_app_cards_v\`;`)
