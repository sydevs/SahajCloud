...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  // Issue #291: Existing `lectures` rows are dropped by design — the collection
  // now models a full talk (was a clip). ON DELETE cascade / set null FKs take
  // care of `lectures_rels` and `wm_app_config_locales.post_realization_lecture_id`
  // automatically.
  await db.run(sql`DELETE FROM \`lectures\`;`)
  ...
  await db.run(sql`INSERT INTO \`__new_app_cards_rels\`("id", "order", "parent_id", "path", "albums_id", "meditations_id") SELECT "id", "order", "parent_id", "path", "albums_id", "meditations_id" FROM \`app_cards_rels\`;`)
  ...
  await db.run(sql`INSERT INTO \`__new__app_cards_v_rels\`("id", "order", "parent_id", "path", "albums_id", "meditations_id") SELECT "id", "order", "parent_id", "path", "albums_id", "meditations_id" FROM \`_app_cards_v_rels\`;`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_lectures\`("id", "nirmal_vidya_vimeo_url", "start_time", "end_time", "thumbnail_id", "video_url", "updated_at", "created_at", "_status") SELECT "id", "nirmal_vidya_vimeo_url", "start_time", "end_time", "thumbnail_id", "video_url", "updated_at", "created_at", "_status" FROM \`lectures\`;`)
  await db.run(sql`DROP TABLE \`lectures\`;`)
  await db.run(sql`ALTER TABLE \`__new_lectures\` RENAME TO \`lectures\`;`)
  await db.run(sql`CREATE INDEX \`lectures_thumbnail_idx\` ON \`lectures\` (\`thumbnail_id\`);`)
  await db.run(sql`CREATE INDEX \`lectures_updated_at_idx\` ON \`lectures\` (\`updated_at\`);`)
