...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  // Drizzle generates `viewer_rules_id` on both sides of this INSERT/SELECT, but
  // the old `payload_locked_documents_rels` only has `lecture_tags_id` — the
  // known polymorphic-FK rename bug documented in CLAUDE.md. Drop the new
  // column from both sides: existing rows pointing at the dropped `lecture_tags`
  // table are already orphaned, and no viewer-rules locks exist yet.
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`lecture_clips_rels_lecture_tags_id_idx\` ON \`lecture_clips_rels\` (\`lecture_tags_id\`);`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_lectures\`("id", "nirmal_vidya_vimeo_url", "thumbnail_id", "video_url", "updated_at", "created_at") SELECT "id", "nirmal_vidya_vimeo_url", "thumbnail_id", "video_url", "updated_at", "created_at" FROM \`lectures\`;`)
  ...
  `)
  // Mirror of the up-migration fix: the live table has `viewer_rules_id` but
  // rolling back creates a new table with `lecture_tags_id`. Drop the renamed
  // column from both sides of the INSERT/SELECT.
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "pages_id", "meditations_id", "songs_id", "albums_id", "videos_id", "lessons_id", "lectures_id", "lecture_clips_id", "frames_id", "narrators_id", "authors_id", "images_id", "files_id", "meditation_tags_id", "song_tags_id", "managers_id", "clients_id", "app_cards_id", "forms_id", "form_submissions_id") SELECT "id", "order", "parent_id", "path", "pages_id", "meditations_id", "songs_id", "albums_id", "videos_id", "lessons_id", "lectures_id", "lecture_clips_id", "frames_id", "narrators_id", "authors_id", "images_id", "files_id", "meditation_tags_id", "song_tags_id", "managers_id", "clients_id", "app_cards_id", "forms_id", "form_submissions_id" FROM \`payload_locked_documents_rels\`;`)
  await db.run(sql`DROP TABLE \`payload_locked_documents_rels\`;`)
  await db.run(sql`ALTER TABLE \`__new_payload_locked_documents_rels\` RENAME TO \`payload_locked_documents_rels\`;`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_order_idx\` ON \`payload_locked_documents_rels\` (\`order\`);`)
