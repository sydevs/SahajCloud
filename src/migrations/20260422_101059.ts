...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  // Like `20260421_030949.ts`, this migration has a partial-apply footprint in
  // prod (D1 has no transactional DDL). Every statement below is idempotent —
  // safe against fresh, partially applied, or fully applied state.

  // Clean up any leftover rebuild tables from prior failed runs.
  await db.run(sql`DROP TABLE IF EXISTS \`__new_lectures\`;`)
  await db.run(sql`DROP TABLE IF EXISTS \`__new_lecture_clips\`;`)
  ...
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`lectures_rels\` (
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lectures_rels_order_idx\` ON \`lectures_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lectures_rels_parent_idx\` ON \`lectures_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lectures_rels_path_idx\` ON \`lectures_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lectures_rels_audiences_id_idx\` ON \`lectures_rels\` (\`audiences_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lecture_clips_rels_order_idx\` ON \`lecture_clips_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lecture_clips_rels_parent_idx\` ON \`lecture_clips_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lecture_clips_rels_path_idx\` ON \`lecture_clips_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lecture_clips_rels_audiences_id_idx\` ON \`lecture_clips_rels\` (\`audiences_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`audiences\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  	\`rules\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`audiences_updated_at_idx\` ON \`audiences\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`audiences_created_at_idx\` ON \`audiences\` (\`created_at\`);`)

  // Rebuild `lectures` to drop the hasOne `audience_id` column (superseded by
  // the new `lectures_rels.audiences_id` join).
  if (await columnExists(db, 'lectures', 'audience_id')) {
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lectures_thumbnail_idx\` ON \`lectures\` (\`thumbnail_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lectures_updated_at_idx\` ON \`lectures\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lectures_created_at_idx\` ON \`lectures\` (\`created_at\`);`)

  // Rebuild `lecture_clips` to drop its hasOne `audience_id` column.
  if (await columnExists(db, 'lecture_clips', 'audience_id')) {
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lecture_clips_parent_idx\` ON \`lecture_clips\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lecture_clips_thumbnail_idx\` ON \`lecture_clips\` (\`thumbnail_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lecture_clips_updated_at_idx\` ON \`lecture_clips\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`lecture_clips_created_at_idx\` ON \`lecture_clips\` (\`created_at\`);`)

  // Rebuild `app_cards` to drop its hasOne `audience_id` column.
  if (await columnExists(db, 'app_cards', 'audience_id')) {
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`app_cards_updated_at_idx\` ON \`app_cards\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`app_cards_created_at_idx\` ON \`app_cards\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`app_cards__status_idx\` ON \`app_cards\` (\`_status\`);`)

  // Rebuild `_app_cards_v` to drop its hasOne `version_audience_id` column.
  if (await columnExists(db, '_app_cards_v', 'version_audience_id')) {
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_parent_idx\` ON \`_app_cards_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_version_version_image_idx\` ON \`_app_cards_v\` (\`version_image_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_version_version_updated_at_idx\` ON \`_app_cards_v\` (\`version_updated_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_version_version_created_at_idx\` ON \`_app_cards_v\` (\`version_created_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_version_version__status_idx\` ON \`_app_cards_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_created_at_idx\` ON \`_app_cards_v\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_updated_at_idx\` ON \`_app_cards_v\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_snapshot_idx\` ON \`_app_cards_v\` (\`snapshot\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_published_locale_idx\` ON \`_app_cards_v\` (\`published_locale\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_latest_idx\` ON \`_app_cards_v\` (\`latest\`);`)

  // Rebuild `payload_locked_documents_rels` to rename the polymorphic FK column
  // `viewer_rules_id` → `audiences_id`. Guard with the old column's existence.
  if (await columnExists(db, 'payload_locked_documents_rels', 'viewer_rules_id')) {
  ...
  `)
    // Drizzle emits `audiences_id` on both sides of this INSERT/SELECT, but the
    // old `payload_locked_documents_rels` only has `viewer_rules_id` — the
    // known polymorphic-FK rename bug documented in CLAUDE.md, already handled
    // the same way in 20260421_112332.ts. Drop the new column from both sides:
    // any viewer_rules locks are already orphaned (viewer_rules is dropped
    // immediately after), and no audience rows exist yet to lock.
    await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "pages_id", "meditations_id", "songs_id", "albums_id", "videos_id", "lessons_id", "lectures_id", "lecture_clips_id", "frames_id", "narrators_id", "authors_id", "images_id", "files_id", "meditation_tags_id", "song_tags_id", "managers_id", "clients_id", "app_cards_id", "forms_id", "form_submissions_id") SELECT "id", "order", "parent_id", "path", "pages_id", "meditations_id", "songs_id", "albums_id", "videos_id", "lessons_id", "lectures_id", "lecture_clips_id", "frames_id", "narrators_id", "authors_id", "images_id", "files_id", "meditation_tags_id", "song_tags_id", "managers_id", "clients_id", "app_cards_id", "forms_id", "form_submissions_id" FROM \`payload_locked_documents_rels\`;`)
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_meditations_id_idx\` ON \`payload_locked_documents_rels\` (\`meditations_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_songs_id_idx\` ON \`payload_locked_documents_rels\` (\`songs_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_albums_id_idx\` ON \`payload_locked_documents_rels\` (\`albums_id\`);`)
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_lessons_id_idx\` ON \`payload_locked_documents_rels\` (\`lessons_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_lectures_id_idx\` ON \`payload_locked_documents_rels\` (\`lectures_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_lecture_clips_id_idx\` ON \`payload_locked_documents_rels\` (\`lecture_clips_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_frames_id_idx\` ON \`payload_locked_documents_rels\` (\`frames_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_narrators_id_idx\` ON \`payload_locked_documents_rels\` (\`narrators_id\`);`)
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_images_id_idx\` ON \`payload_locked_documents_rels\` (\`images_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_files_id_idx\` ON \`payload_locked_documents_rels\` (\`files_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_audiences_id_idx\` ON \`payload_locked_documents_rels\` (\`audiences_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_meditation_tags_id_idx\` ON \`payload_locked_documents_rels\` (\`meditation_tags_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_song_tags_id_idx\` ON \`payload_locked_documents_rels\` (\`song_tags_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_managers_id_idx\` ON \`payload_locked_documents_rels\` (\`managers_id\`);`)
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_app_cards_id_idx\` ON \`payload_locked_documents_rels\` (\`app_cards_id\`);`)
  ...
  await db.run(sql`DROP TABLE IF EXISTS \`viewer_rules\`;`)

  if (!(await columnExists(db, 'app_cards_rels', 'audiences_id'))) {
    await db.run(sql`ALTER TABLE \`app_cards_rels\` ADD \`audiences_id\` integer REFERENCES audiences(id);`)
  }
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`app_cards_rels_audiences_id_idx\` ON \`app_cards_rels\` (\`audiences_id\`);`)
  if (!(await columnExists(db, '_app_cards_v_rels', 'audiences_id'))) {
    await db.run(sql`ALTER TABLE \`_app_cards_v_rels\` ADD \`audiences_id\` integer REFERENCES audiences(id);`)
  }
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`_app_cards_v_rels_audiences_id_idx\` ON \`_app_cards_v_rels\` (\`audiences_id\`);`)
}
