...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  // Detect current state to handle partial migration from a previous failed deploy.
  // A previous deploy of PR #361 succeeded at the RENAME COLUMN statements but failed at the
  // INSERT SELECT (which incorrectly referenced the new column names that didn't exist yet).
  const localeCols = new Set(
    ((await (db as any).all(sql`SELECT name FROM pragma_table_info('app_cards_locales')`)) as { name: string }[]).map(r => r.name)
  )
  const appCardsCols = new Set(
    ((await (db as any).all(sql`SELECT name FROM pragma_table_info('app_cards')`)) as { name: string }[]).map(r => r.name)
  )

  const headerExists = localeCols.has('header')
  const imageIdExists = appCardsCols.has('image_id')

  // If both old columns are gone, the migration was fully applied but not recorded — skip.
  if (!headerExists && !imageIdExists) return

  // === RENAME COLUMNs in app_cards_locales (skip if already done from previous partial run) ===
  if (headerExists) {
  ...
  // === RENAME COLUMNs in _app_cards_v_locales (skip if already done) ===
  const vLocaleCols = new Set(
    ((await (db as any).all(sql`SELECT name FROM pragma_table_info('_app_cards_v_locales')`)) as { name: string }[]).map(r => r.name)
  )
  if (vLocaleCols.has('version_header')) {
  ...
  // === Rebuild app_cards table (only if not yet rebuilt) ===
  // The original migration's INSERT SELECT incorrectly read "default_image_id" etc. from the old
  // app_cards table. The correct old column names are image_id, overlay, app_page.
  if (imageIdExists) {
  ...
  // === ADD COLUMNs to app_cards_locales (skip if already added) ===
  const updatedLocaleCols = new Set(
    ((await (db as any).all(sql`SELECT name FROM pragma_table_info('app_cards_locales')`)) as { name: string }[]).map(r => r.name)
  )
  if (!updatedLocaleCols.has('starting_soon_header')) {
  ...
  // === Rebuild _app_cards_v table ===
  // Fixed: SELECT uses old column names (version_image_id, version_overlay, version_app_page).
  const appCardsVCols = new Set(
    ((await (db as any).all(sql`SELECT name FROM pragma_table_info('_app_cards_v')`)) as { name: string }[]).map(r => r.name)
  )
  if (appCardsVCols.has('version_image_id')) {
  ...
  // === ADD COLUMNs to _app_cards_v_locales (skip if already added) ===
  const updatedVLocaleCols = new Set(
    ((await (db as any).all(sql`SELECT name FROM pragma_table_info('_app_cards_v_locales')`)) as { name: string }[]).map(r => r.name)
  )
  if (!updatedVLocaleCols.has('version_starting_soon_header')) {
  ...
  // === Rebuild app_cards_rels (drops old lecture/album/meditation FKs, keeps audiences only) ===
  ...
  // === DROP COLUMNs from audiences (skip if already dropped) ===
  const audiencesCols = new Set(
    ((await (db as any).all(sql`SELECT name FROM pragma_table_info('audiences')`)) as { name: string }[]).map(r => r.name)
  )
  if (audiencesCols.has('event_time')) {
