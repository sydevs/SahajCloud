 ...
 * registrant confirm/deny vote tallies for an unverified listing — plus the
 * verdict math that turns those tallies into a ranking score and an unpublish
 * decision. Written by the Registrations vote-sync hook (alongside the indexed
 * `confidenceScore` column, which stays a real column because the feeds sort
 * on it); read by the admin verification notice.
 *
 * The stored shape is declared as a JSON Schema rather than parsed by a
 * hand-written reader: Payload generates the TypeScript type from it — so
 * `event.systemMeta?.communityFeedback?.denials` is checked at compile time —
 * and validates on write, so a malformed tally can't reach the column in the
 * first place. A defensive runtime reader could only report that the data had
 * already landed wrong. Same mechanism as `Registrations.questions`.
 */
