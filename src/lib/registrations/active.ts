 ...
 * `rejected` buys is exactly this exclusion — the registrant stops receiving
 * reminders, and they stop occupying a seat.
 *
...
export function isActiveRegistration(submission: {
  ...
  return submission.type === 'registration' && submission.status !== 'rejected' && submission.status !== 'failed'
}
