  ...
  return {
    ...
    handler: async (req) => {
      ...
      if (result.status === 'expired') {
        return noticePage(410, 'This link has expired', 'Request a new sign-in link.', requestPagePath)
      }
      if (result.status !== 'valid' || result.claims.collection !== slug) {
        return noticePage(400, 'This link is not valid', 'Request a new sign-in link.', requestPagePath)
      }
