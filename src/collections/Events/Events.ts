...
export const Events: CollectionConfig = {
  ...
  // ⚠ Never add `localizeStatus` here. Every bookkeeping write — a registration,
  // a feedback vote, the nightly sweep — goes through
  // `updateEventWithoutValidation`, which passes `unpublishAllLocales` to skip
  // validation. That argument writes `_status: 'draft'` for every locale only on
  // a collection which sets this flag, turning each of those writes into a mass
  // unpublish. The root `experimental.localizeStatus` flag is already on, so
  // nothing else stops it. Held by `tests/unit/event-localize-status-guard.spec.ts`.
