...
- **The published-only filter is now per locale.** A client restricted to published documents reads a locale only where that locale is `published` — so an unpublished translation's text is unreachable, and a page published in English alone returns nothing at `?locale=de`. On existing data nothing changes: the migration that landed this carried every document's old status into all 19 locales, so the narrowing only appears once an editor unpublishes a locale on purpose.
