...
- ⚠ **That second case fires on existing data, not only on a deliberate unpublish.** The migration copies each document's old status into every locale row that *exists*, and an untranslated locale has none — Payload's own `localizeStatus` migration template has the same property, since it `UPDATE`s locale rows and never inserts one. So a page published in English alone returns nothing at `?locale=de`, where today Payload's English fallback serves it. A consumer that renders an untranslated locale by fallback must adapt first ([WeMeditateWeb#81](https://github.com/sydevs/WeMeditateWeb/issues/81)).
- ⚠ **A `locale=all` read by such a client returns `_status` and `id`, and nothing else.** The access filter matches *documents* — a document published in any locale — while the response would otherwise carry every locale's value for every field, including a locale the editor has taken down. So a cross-locale client read answers which locales are published and stops there (`withPublishedLocaleRedaction`, `src/plugins/access/`). Read content one locale at a time, where the filter gates it properly. A manager, the admin panel, and a live-preview request are unaffected.
