...
⚠ **`managers` is there because "no project" was reading as shared** (#821): every published API key, the Atlas widget's browser key included, read every manager's name and email — directly, and through `Events.manager`, `Regions.managers`, `Pages.managers` and `Clients.managers` at `depth >= 1`. **No client role gets a grant back.** `atlas-manager` holds the only one, because it picks on `Events.manager` and `Regions.managers`; `web-translator` gets nothing and renders a raw id in the Page Editors sidebar, which is the accepted cost. A denormalized *copy* of a manager's address is a separate problem this entry cannot solve — see `Events.registrationNotificationEmail` in the next section.
...
It is deliberately **wider** than `adminOnlyFieldAccess`, which is the wrong tool for this: a client's own managers must be able to configure their service, and document-level manager access already decides which clients each manager sees. Four fields carry it. `Clients.mailingList` — `clients` is not restricted, so without the lock every published key could read every service's provider secret (`docs/rules/api-clients.md`). `Forms.recipient` and `Forms.client`, on a collection that keeps the form-builder plugin's `read: () => true`, which outranks the generated access config: populating a `clients` document hands over a plaintext `apiKey` (#822), and `recipient`'s half is now defence in depth behind restricted `managers`. And `Events.registrationNotificationEmail` — ⚠ **a lock `RESTRICTED_COLLECTIONS` could not have supplied**, because the field is a manager's address *copied* onto a collection the `sahaj-atlas` project reads, not a relationship to the manager (#821). Reach for this lock, not the list, whenever personal data is denormalized rather than related. Its sibling `Events.contactEmail` stays open: that one is the address a seeker is meant to write to. ⚠ **The field still appears in `payload-types.ts`.** The lock strips the value at runtime, not the shape from the generated type.
