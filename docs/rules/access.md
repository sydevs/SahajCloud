...
| `SubmissionReview/urls.ts` → `/api/user-submissions/:id/review` | **Yes** | `applyReview`'s role check does |
...
- **Live preview is untouched.** It reads drafts through `find`/`findByID` with `draft: true`, which resolves against `read` and the live-preview branch — never through a versions operation.
...
**`RESTRICTED_COLLECTIONS`** (`config/projects.ts`) is the only way to stop that. A collection named there is skipped by implicit read, so only an explicit `read` grant, or the admin bypass, reaches it. It holds `users`, `user-submissions` and `clients` — personal data and credentials.

⚠ **Treat the list as a holding pattern.** Only four collections sit in no project, and `managers` (#821) is the one left outside the set — so the shared-read default is close to having no legitimate beneficiary at all. The real fix is for step 4a to test project membership directly, so a new collection fails closed. Its docblock says what blocks that.
...
`RESTRICTED_COLLECTIONS` is collection-wide, and sometimes only one field must be hidden from a client — on a collection the client legitimately reads, or on the one row restriction never covers, its own. `managersOnlyFieldAccess` (`@/plugins/access`) is that lock: it denies any caller whose `req.user.collection === 'clients'`, read and write alike.

It is deliberately **wider** than `adminOnlyFieldAccess`, which is the wrong tool for this: a client's own managers must be able to configure their service, and document-level manager access already decides which clients each manager sees. Four fields carry it. ⚠ **`Clients.apiKey`** — the base auth field Payload decrypts on `afterRead`, so without the lock `GET /api/clients/me` hands a browser-shipped key its own plaintext credential, and restricting the collection cannot stop it (#822). Declare it at the **top level** of `Clients.fields`: `mergeBaseFields` matches by name only at the level it is handed, so a copy nested in a tab sanitizes to two `apiKey` fields instead of one. And `Clients.mailingList` — without the lock a service's own key could read back its provider secret (`docs/rules/api-clients.md`). And `Forms.recipient` and `Forms.client`, on a collection that keeps the form-builder plugin's `read: () => true`: `managers` sits in no project, so a widget read at `depth >= 1` would populate the manager and hand a name and an email address to a browser. `Forms.client` is now belt and braces — restricting `clients` already stops that populate — and it stays, because it is the half that survives if the collection is ever unrestricted. ⚠ **The field still appears in `payload-types.ts`.** The lock strips the value at runtime, not the shape from the generated type.

⚠ **A field lock only covers a read that checks access, and not every self-read does.** Payload's `refreshOperation` re-reads the document with `findByID` and no `overrideAccess: false`, so it defaults to `true` and every field lock is skipped — `meOperation` passes the flag, `refresh` does not. `POST /api/clients/refresh-token` handed a browser-shipped key its own decrypted `apiKey` and its `mailingList` provider secret, with both locks in place (#822). `disableLocalStrategy` does not close it: `refresh` is the one auth operation that does not refuse on that flag. `stripSecretsFromClientReads`, a collection `afterRead` hook keyed on `req.user.collection === 'clients'`, is what does — a hook runs on every read path, so it holds whichever operation forgets the flag next. It strips every field carrying its own `read` lock, read off the sanitized collection, so a fifth locked field needs no edit there. **When you lock a field on an auth collection, lock it and then check what else re-reads the row.**
...
| **No manager role may read it** | Grant it in **no** role |
