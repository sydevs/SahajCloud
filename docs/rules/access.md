...
⚠ **All four are hooks on an auth *response*, so a sign-in that builds its own response gets none of them.** That is why `GET /api/managers/consume-link` (#837) **redirects** to `/admin` instead of returning a user: a hand-built body there would carry flat, single-locale `roles` that nothing else in the app hands out. The redirect leaves `afterMe` to resolve them on the admin's next `/me`. Any future route that mints a session itself owes the same choice — return nothing, or run the roles through `withLocalizedRoleAuth`'s shape.
