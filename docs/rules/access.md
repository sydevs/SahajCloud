...
It is deliberately **wider** than `adminOnlyFieldAccess`, which is the wrong tool for this: a client's own managers must be able to configure their service, and document-level manager access already decides which clients each manager sees. Two fields carry it. `Clients.mailingList` — `clients` is not restricted, so without the lock every published key could read every service's provider secret (`docs/rules/api-clients.md`). And `Forms.recipient`, whose collection keeps the form-builder plugin's `read: () => true`: `managers` sits in no project, so a widget read at `depth >= 1` would populate the manager and hand a name and an email address to a browser. ⚠ **The field still appears in `payload-types.ts`.** The lock strips the value at runtime, not the shape from the generated type.
