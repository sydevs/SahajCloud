...
Two limits worth knowing. **It collapses the list-level resolutions only.** `createAccessConfig`'s single-document `update` branch (`id` present) routes to `userManagesDocument` instead, which is still unmemoized — so a document edit view, where `getDocumentPermissions` repeats the whole operation set four times for a drafts+trash collection, still pays that path per evaluation. And ⚠ **the memo pins the managed set for the life of `req`**: a request that writes a `managers` field and then makes a *second* access decision on that same `req` reads the pre-write set. `documentManagers.ts` carries the rest, including why the memo must not be lifted to the access function.
