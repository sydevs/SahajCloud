...
- **The root's copy is operator-written, on `sy-atlas-translations`** (`seo.root_title`, `seo.root_description`) — the global that is already localized and already seeded in ten locales. Its `description` is `null` in any locale nobody wrote one for, exactly as a region's is; only its `title` falls back to English and then to a constant, because `<title>` is mandatory markup and a blank one is worse than one in the wrong language. That read deliberately bypasses `clientEnglishFallback`, which would otherwise defeat the `description` rule (`src/endpoints/atlas/seo/rootStrings.ts`).
- **The root's canonical is the caller's own verified embed page**, read from its `clients` row and never recomputed — the mount URL with no route appended, identical in both routing modes. A caller that owns no canonical falls through to the We Meditate surface, exactly as an unclaimed region does.
