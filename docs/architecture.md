...
Three differences follow from the guard denying rather than throwing. It captures a **message** (`Sentry.captureMessage`) where the hook captures the exception, since there is no exception. It is **not gated on `NEXT_PUBLIC_SENTRY_DSN`** — `sentryPlugin` returns the config untouched without one, so the hook's WARN mirror disappears with it, while the guard's line survives a local run. And its capture is **deduped per key fingerprint**, at most one event per minute per process: the hook's path needs a Payload route that throws, while these fifteen endpoints are public and the edge allows 500 req/min per (client, IP), so one stale integration could otherwise post ~720k `error` events a day. The bound sits here rather than in a Sentry rule because that rule is external to this repo and may not exist, and because muting by dropping the level would discard the signal. ⚠ **The WARN line is deliberately not deduped** — it is cheap, it needs no DSN, and per-request diagnosability is the reason it exists. Reporting there never throws, and the log has its own `try` so a broken transport cannot cost the Sentry event: a telemetry failure must not turn a caller's 403 into a 500.
