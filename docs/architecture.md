...
- **The root's copy is operator-written, on `sy-atlas-translations`** (`seo.root_title`, `seo.root_description`) — the global that is already localized and already seeded in ten locales. Its `description` is `null` in any locale nobody wrote one for, exactly as a region's is; only its `title` falls back to English and then to a constant, because `<title>` is mandatory markup and a blank one is worse than one in the wrong language. That read opts out of the `clientEnglishFallback` merge with `withoutEnglishFallback(req)`, which would otherwise defeat the `description` rule (`src/endpoints/atlas/seo/rootStrings.ts`).
