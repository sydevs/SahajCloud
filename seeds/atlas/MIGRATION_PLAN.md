...
- **Venues eliminated**: a venue referenced by >1 event becomes a Regions `venue` node; a single-use venue's address is lifted onto its Event (`address` group). Street addresses live on Events, not on geo nodes. At import, a venue's Google `place_id` resolves to an OSM id.
...
> **Phase 2 update:** there is **no Venues collection**. Venues referenced by >1 event become Regions `venue` nodes; single-use venues' addresses are lifted onto their Event's `address` group. This `venues.json` extract still feeds the Phase 3 importer, which routes each venue to a shared-venue node or an event address.
...
- **Payload mapping (Phase 2/3, #462):** `clientId` ← `public_key` (readOnly); `active` ← `enabled`; `domains` ← `config.domain`; `color1` ← `config.primary_color` (`color2`/`color3` ship empty — not in the extract); `locale` ← `config.locale` (ISO-639-1, any language); `region` ← `location` geo ref → `regions` doc. **`config.routing_type` / `embed_type` / `default_view` are not mapped** — #633 removed the `legacyConfig` field rather than promoting it, because a second contradictory source of routing/embed data beside the new `canonical` group is how the two drift (and the legacy values are demonstrably wrong: `sahajayoga.at` records `script` while serving an iframe). The raw values survive verbatim in `legacyData.config`, which is what `scripts/backfill-client-canonical.ts` seeds `canonical.domain` / `canonical.routing` from — always leaving `canonical.enabled` false.
