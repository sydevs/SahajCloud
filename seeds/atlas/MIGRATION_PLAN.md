...
- **Payload mapping (Phase 2/3):** legacy `defaultLanguageCode` (country) → `eventDefaults.language`; `timeZone` (area, multi) → `eventDefaults.timeZone`. Both sit in the Regions **`eventDefaults`** group and **inherit down the tree** — a collection-level afterRead hook fills a blank `language`/`timeZone` from the nearest ancestor that sets it (Country → Region → Area → Center), each independently. `countryCode` is used only for area→country parent resolution at import and is **not stored** on the node; `osm_id` → the required `osmId` field (sentinel `custom` = manual `latitude`/`longitude`/`radius`).
