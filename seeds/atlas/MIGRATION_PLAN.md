...
- **Payload mapping (Phase 2/3, #462):** `clientId` ← `public_key` (readOnly); `active` ← `enabled`; `domains` ← `config.domain`; `color1` ← `config.primary_color` (`color2`/`color3` ship empty — not in the extract); `locale` ← `config.locale` (ISO-639-1, any language); `region` ← `location` geo ref → `regions` doc; `legacyConfig` ← `{ routing_type, embed_type, default_view }` (readOnly json — `config.default_view` is folded in here, not separately dropped).
  - **Superseded by #633.** `legacyConfig` is gone — the field, the column, and the importer write. Keeping a second, contradictory source of `routing_type`/`embed_type` beside the new `canonical.*` fields is how they drift, and the legacy values are unverified (`sahajayoga.at` is recorded as `script` and in fact serves an iframe). The whole `config` object still survives verbatim in `legacyData`, which is what `scripts/backfill-client-canonical.ts` seeds `canonical.domain` / `canonical.routing` from — always leaving `canonical.enabled` false.
