...
| File                            | Purpose                                                                          |
| ------------------------------- | -------------------------------------------------------------------------------- |
| `tests/e2e/*.e2e.spec.ts`       | REST flows that only a deployed environment can answer: auth, content CRUD, CORS preflight, and what an error body discloses under the shipped `config.debug` (`error-disclosure`) |
