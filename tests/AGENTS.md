...
Reserve the full `pnpm test:int` / `pnpm build` for reproducing a red CI check. See [Test execution requirements](#test-execution-requirements) below for the local-vs-CI split.
...
>
> The gate only earns its keep if fixtures are typed against the real schema — see **"Typed fixtures"** above.
...
Coverage map: `tests/COVERAGE.md`.
