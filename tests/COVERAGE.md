...
| `frames`              | `cascadeFrameNodeChange` afterChange. Meditation-frame validation / rounding / sorting / enrichment hooks. Node-weights cache persistence: a cascaded weight surviving a later save that recombines from the pre-cascade version row (#843) | `meditationFrames`                                             |
...
| `/api/atlas/seo` (root endpoint)      | Route → region/event resolution. Canonical read from each document's own `webUrl` and each breadcrumb rung's own ancestor's. Descendant-inclusive class listing. 404 matrix, including which refusals are a 400 on the schema and which a 404 on the parse. The atlas root and every bare view route answering one document, its seeded copy per locale, and `description: null` rather than English where a locale has none (#739). Locale-free canonical across locales. Ordered image list driving `og:image`. The operator-owned hreflang set changing live with `sy-atlas-config.availableLocales`, and English alone when it is unconfigured. JSON-LD script-breakout safety end to end | `atlas-seo` + unit: `atlas-seo-route.spec.ts`, `atlas-seo-document.spec.ts`, `atlas-seo-root-strings.spec.ts`, `available-locales.spec.ts` |
...
| `POST /api/user-submissions/:id/review` | The locale gate over the real REST pipeline: an `atlas-manager` holding roles only in French is denied with no `?locale=`, allowed at `?locale=fr`, denied at `?locale=en` (#701), plus the `type` gate refusing a non-proposal row | `user-submissions-review` |
| Proposal review lifecycle | The two virtual projections, reopen, and `applyReview`'s transitions on `user-submissions` — including a `failed` row still being actionable | `user-submissions-review-lifecycle` |
...
| `ExpireEvents`                   | Finished-check + reminder ladder. Publish side effects. Per-event failure isolation. The `EVENT_VERIFICATION_ENABLED` pause. An event whose **stored** data fails a field validator (#835) — advanced, finished, and its reminder logged once — plus the `latest: true` version row matching the main row after a write, and the repeat-failure tag. Those cases seed the invalid state at the db layer, since the public API refuses it — `tests/utils/storeInvalidEvent.ts`, shared with the three specs that also need that shape (#842) | `expire-events` + unit: `expire-events-stage-machine.spec.ts`, `expire-events-flag.spec.ts`, `schedule-status.spec.ts` |
| `SyncLectureMetadata`            | Batch sync, NV API error isolation, filtering                             | `sync-lecture-metadata`                                                 |
| `screenSubmission`               | The uniform base per type (disposable, MX fail-open, cross-intake history), proposal content screening of `proposed`, and the hand-off: a clean verdict queues delivery, a refusal queues none. **Abuse counting reads the verdict, not the status** — two senders with identical six-`rejected` histories, differing only in who refused them. A refused registration is flagged and drops out of `activeRegistrationWhere`. A registration opt-in spawns its linked subscribe row | `screen-submissions` + unit: `submission-screening.spec.ts`, `registration-active.spec.ts` |
