...
| `requireActiveClient` reporting the rejected credential its returned 403 hides from every error hook, staying silent for an anonymous or authenticated denial, capturing one event per key fingerprint per minute while still logging every denial, and never costing the caller its 403 or its event (#743) | unit: `endpoint-guard-credential-rejected.spec.ts` |
