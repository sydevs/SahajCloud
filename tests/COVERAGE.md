...
| `ExpireEvents`                   | Finished-check + reminder ladder. Publish side effects. Per-event failure isolation. The `EVENT_VERIFICATION_ENABLED` pause | `expire-events` + unit: `expire-events-stage-machine.spec.ts`, `expire-events-flag.spec.ts`, `schedule-status.spec.ts` |
