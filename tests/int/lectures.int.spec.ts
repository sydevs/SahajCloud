...
describe('Lectures Collection', () => {
  ...
  describe('lastRefreshed field', () => {
    it('is set on create', async () => {
      const { fetchNirmalaVidyaVideo } = await import('@/lib/nirmalaVidyaApi')
      vi.mocked(fetchNirmalaVidyaVideo).mockResolvedValueOnce({
        title: 'Timestamped Lecture',
        thumbnailUrl: 'https://example.com/thumb.jpg',
        hlsUrl: 'https://example.com/stream.m3u8',
      })
      ...
      expect(lecture.lastRefreshed).toBeDefined()
      // lastRefreshed should be a recent timestamp
      expect(new Date(lecture.lastRefreshed!).getTime()).toBeGreaterThanOrEqual(
  ...
  describe('Refresh endpoint logic', () => {
    /**
     * These tests simulate the refresh endpoint logic by testing the same
     * operations the handler performs: load lecture, extract Vimeo ID,
     * fetch fresh data from API, and return updated values.
     */

    it('returns updated field values for a valid lecture', async () => {
      const { fetchNirmalaVidyaVideo, extractVimeoId } = await import('@/lib/nirmalaVidyaApi')

      const lecture = await testData.createLecture(payload)

      // Simulate refresh: load lecture, extract Vimeo ID, fetch API data
      const loaded = await payload.findByID({
        collection: 'lectures',
        id: lecture.id,
        depth: 0,
      })
      expect(loaded.nirmalVidyaVimeoUrl).toBeDefined()

      const vimeoId = extractVimeoId(loaded.nirmalVidyaVimeoUrl!)
      expect(vimeoId).toBe('123456789')

      vi.mocked(fetchNirmalaVidyaVideo).mockResolvedValueOnce({
        title: 'Refreshed Title',
        thumbnailUrl: 'https://example.com/new-thumb.jpg',
        hlsUrl: 'https://example.com/new-stream.m3u8',
      })

      const videoData = await fetchNirmalaVidyaVideo(vimeoId!)
      expect(videoData.title).toBe('Refreshed Title')
      expect(videoData.hlsUrl).toBe('https://example.com/new-stream.m3u8')
    })

    it('cannot refresh a lecture without a Vimeo URL', async () => {
      const { extractVimeoId } = await import('@/lib/nirmalaVidyaApi')

      // Create a lecture then manually clear the URL field via DB won't work
      // since access.update blocks it, but we can verify the logic:
      // If nirmalVidyaVimeoUrl is empty, extractVimeoId returns null
    ...
    it('handles API failure gracefully', async () => {
      ...
      vi.mocked(fetchNirmalaVidyaVideo).mockRejectedValueOnce(
        new Error('Nirmala Vidya API is temporarily unavailable.'),
      )

      await expect(fetchNirmalaVidyaVideo('123456789')).rejects.toThrow(
        'Nirmala Vidya API is temporarily unavailable.',
      )
    })

    it('returns 404 when lecture does not exist', async () => {
      // Simulate endpoint logic: findByID throws for non-existent IDs
      await expect(
        payload.findByID({
          collection: 'lectures',
          id: 99999,
