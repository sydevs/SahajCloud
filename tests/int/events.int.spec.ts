...
describe('Events collection', () => {
  ...
  describe('website field', () => {
    ...
    // Proves the urlField() validator is actually wired onto this field;
    // tests/unit/url-field.spec.ts covers the validator's own messages.
    it('rejects a value that is not an http(s) URL', async () => {
      await expect(testData.createEvent(payload, { website: 'not-a-url' })).rejects.toThrow(
        /Details > Website/,
      )
    })
  })
