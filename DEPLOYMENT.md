...
5. **Configure Workers Builds** in the CF dashboard (Workers & Pages → `sahajcloud` → Settings → Builds):
   ...
   - **Pre-deploy migration step: `pnpm run deploy:database:preview`.** This project uses Payload migrations (`src/migrations/*.ts` via `payload migrate`, tracked in `payload_migrations`), NOT Cloudflare's native `wrangler d1 migrations apply` (which expects `.sql` files in a `migrations_dir` and a `d1_migrations` table — neither exists here; that command applies nothing). `deploy:database:preview` runs `CLOUDFLARE_ENV=preview payload migrate` against the remote `sahajcloud-preview` D1 so the preview always matches the deployed code. **Run it here in Workers Builds, not in GitHub Actions:** `payload migrate`'s remote bindings open a Workers edge-preview session (`getPlatformProxy({ remoteBindings: true })`), which needs a credential with **Workers Scripts:Edit** scope. Workers Builds runs with account-scoped credentials that satisfy this; the GitHub `CLOUDFLARE_API_TOKEN` (D1+R2 only, for `preview-reclone`) does **not** — running the step there fails with `Authentication error [code: 10000]` on `/workers/subdomain/edge-preview`. (If you'd rather run it from CI, broaden that token to include Workers Scripts:Edit first.)
